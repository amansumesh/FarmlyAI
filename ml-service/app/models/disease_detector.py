...
class DiseaseDetectionModel:
    ...
    def __init__(self, num_classes: int = None, model_path: str = None):
        """
        ...
        # Auto model path if not provided
    ...
    def _load_classes(self) -> List[Dict]:
        ...
        if not os.path.exists(classes_path):
            raise FileNotFoundError(f"disease_classes.json not found at {classes_path}")
    ...
    def load_model(self):
        ...
        if os.path.exists(self.model_path):
            print(f"Loading trained model weights from {self.model_path}")
            state_dict = torch.load(self.model_path, map_location=self.device)
            self.model.load_state_dict(state_dict)
            print("✅ Model loaded successfully")
        else:
            print("⚠️ disease_model.pth not found. Using pretrained MobileNetV2 only.")
    ...
    def predict(self, image_tensor: torch.Tensor, top_k: int = 3) -> List[Dict]:
        ...
        Args:
            image_tensor: Preprocessed image tensor (must be shape [1,3,224,224])
    ...
    def _calculate_severity(self, confidence: float, class_info: Dict) -> str:
        ...
        if class_info["disease"].lower() == "healthy":
            return "none"
